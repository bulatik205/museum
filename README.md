...
```
php museum conf -l|-n
php museum cong -d:--now_--type_--randint_my-word (можно собрать свое название)
...
-d              - управление папкой, в которую будет утсановлен скелет
    ...
    --type      - название папки: подставляет тип скелета (web, tbot...)
    --randint   - название папки: просто случайное число от 1.000 до 9.999
    --auto      - название папки: генерируется bin2hex(random_bytes(8)) 
```

<p>На выходе конфигурация <code>-d:/--now\_/--type\_/--randint\_my-word</code> выдаст:</p> 

```
Название сгенерированной папки: 01.01.1970_web_5555_my-word
```